...
Let's say we want to study rainfall at a particular point. We'll use UT Austin in Austin as our example:

```{julia}
target_location = "UT Austin"  		 # <1>
target_lon = -97.760329              # <2>
target_lat = 30.3203890              # <3>
```

1. Descriptive name for our target location (change this for different places!)
2. UT Austin's longitude coordinate (negative means west of Greenwich)
3. UT Austin's latitude coordinate (positive means north of equator)
...
function plot_time_series(station_row, rainfall_df)
	...
	scatter!(ax, rainfall_df.date, rainfall_df.rainfall,  # <4>
		markersize = 50, marker = :circle, strokewidth = 2, color = :transparent)
...
plot_nearby_comparison(stations, rainfall_data; n_stations = 3)
```
...
μ = 3    # Location parameter - shifts the distribution left/right
σ = 0.8    # Scale parameter - controls spread (must be > 0)
...
*The weather trend for Rice-university seems to be following a positive trend. While not apparently noticeable, the average of the max-values of the rainfall seem to be increasing for rainfall near Rice University. I also think the major contribution in this shift are a few values of very high rainfall after 1990s, thus moving the average higher.*
...
*In Austin-Camp Mabry (79-0086), there is a duration (1960-1985) where we observe a decline in the average maximum rainfall. This suggests a negative trend; a decrease in the average of the maximum rainfall values.*
...
- Final: $\mu = 3.0$, $\sigma = 0.8$, $\xi = 0.15$
...
*Increasing the $\mu (6) shifted the GEV curve upwards. The fit got worse and I had to reduce it to 2 and then increase it to 3.*
...
*Increasing the $\sigma seemed to increase the right side of values, i.e. increase the values at higher return periods. Reducing it to 0.8 flattened the curve significantly and now it aligns much better with the data.*
...
*The $\xi value changed the shape of the curve itself. At positive $\xi, the curve appears to be 'convex up' at higher return periods and during negative $\xi, its 'concave down' at higher return periods.*
...
*The biggest deviations are the points with low return values. Manually fitting, they are quite difficult to fit. Given the curve is plotted in logarithmic x-axis, the points with lower return values also appears to be a large cluster of points not being fit well.* 
...
*The initial region of the curve seems to be concave upward no matter what parameter is changed. I believe it is simply an artifact of the log scale on the x axis.*
